...
public class GuiApp extends JFrame {
	...
	public void initListeners() {
		...
		delayButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				List<Flight> flightsList = airport.getFlights();
				DelayFlightDialog delayFlightDialog = new DelayFlightDialog(GuiApp.this, flightsList);
		...
		quitButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// Yes/no pop-up
				int choice = JOptionPane.showConfirmDialog(null, "Do you want to quit?", "Quit",
	...
	public void addFlightToTerminal(Flight flight, String terminal) {
		Terminal tTerminal = obtainTerminal(terminal);
		tTerminal.addFlight(flight);
	}



	public void addFlightToGate(Flight flight, String terminal, int gateNumber) {
		Gate gGate = obtainGate(terminal, gateNumber);
		gGate.addFlight(flight);
	}
	
	public void delayFlight(Flight pFlight) {
		Flight delayedFlight = airport.getFlight(pFlight.getCompany() + pFlight.getFlightNumber());
		delayedFlight.setStatus(pFlight.getStatus());
		delayedFlight.setDepartureTime(pFlight.getDepartureTime());
		
		String terminal = delayedFlight.getGate().substring(0, 1);
		int gateNumber = Integer.parseInt(delayedFlight.getGate().substring(2,3));
		
		Gate gGate = obtainGate(terminal, gateNumber);
		Terminal tTerminal = obtainTerminal(terminal);
		
		airport.notifyObservers();
		tTerminal.notifyObservers();
		gGate.notifyObservers();
	}
	


	public void appendToDisplayArea(String string) {

	}
	
	private Terminal obtainTerminal(String terminal) {
		Terminal tTerminal;
		switch (terminal) {
		case "A":
			tTerminal = termA;
	...
	private Gate obtainGate(String terminal, int gateNumber) {
		Gate gGate;
		switch (terminal) {
		case "A":
			gGate = gatesA[gateNumber - 1];
			break;
		case "B":
			gGate = gatesB[gateNumber - 1];
			break;
		case "C":
			gGate = gatesC[gateNumber - 1];
			break;
		default:
			gGate = gatesA[gateNumber - 1];
			break;
